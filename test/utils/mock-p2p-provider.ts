...
export class MockP2PProvider implements P2PProvider {
    public write = (_: Uint8Array) => {
        assert.fail('Unexpected call to P2PProvider.send');
    };
    public end = () => {
        assert.fail('Unexpected call to P2PProvider.end');
    };
    public destroy = (_?: Error) => {
        assert.fail('Unexpected call to P2PProvider.destroy');
    };
